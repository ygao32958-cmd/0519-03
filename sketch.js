...
function drawChoiceCard(label, move, x, y, isPlayer, hasGlow) {
  ...
  if (isPlayer && capture) {
    ...
    if (capture.width > 0) {
      ...
      scale(-1, 1); // 鏡像
      let s_val = max(300 / capture.width, 420 / capture.height);
      imageMode(CENTER);
      image(capture, 0, 0, capture.width * s_val, capture.height * s_val);
      ...
      if (detections && detections.multiHandLandmarks && detections.multiHandLandmarks.length > 0) {
        ...
        let vw = capture.width * s_val;
        let vh = capture.height * s_val;
        ...
        if (landmarks) {
          for (const edge of HAND_CONNECTIONS) {
            ...
            line((startNode.x - 0.5) * vw, (startNode.y - 0.5) * vh, (endNode.x - 0.5) * vw, (endNode.y - 0.5) * vh);
          }
